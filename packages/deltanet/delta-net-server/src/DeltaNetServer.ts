...
export class DeltaNetServer {
  ...
  public removeWebSocket(webSocket: WebSocket) {
    ...
    if (!this.disposed && this.opts.onLeave) {
      const internalConnectionId = deltaNetV01Connection.connectionId;
  ...
  public validateJoiner(
    ...
    if (this.opts.onJoiner) {
      const rawResult = this.opts.onJoiner({
        ...
        states,
        token,
        internalConnectionId: deltaNetV01Connection.connectionId,
      });
  ...
  public addAuthenticatedConnection(deltaNetV01Connection: DeltaNetV01Connection): void {
    if (deltaNetV01Connection.connectionId === null) {
    ...
    this.connectionIdToDeltaNetServerConnection.set(
      deltaNetV01Connection.connectionId,
      deltaNetV01Connection,
    );
  ...
  public validateAndApplyStateUpdate(
    ...
    if (deltaNetV01Connection.isObserver) {
      return new DeltaNetServerError("Observers cannot send state updates", false);
    }
  ...
  private clearInternalConnectionId(internalConnectionId: number) {
    const index = this.connectionIdToComponentIndex.get(internalConnectionId);
    if (index === undefined) {
    ...
    this.connectionIdToComponentIndex.delete(internalConnectionId);
  ...
  public tick() {
    if (this.disposed) {
      return; // Silently ignore ticks after disposal
    }
  ...
  public setUserComponents(
    ...
    // States are now handled individually in the connection, so we don't process them here
    // The states parameter is kept for backward compatibility but ignored
  ...
  private applyComponentUpdates(
    ...
    for (const [componentId, componentValue] of components) {
      ...
      collection.setValue(index, BigInt(componentValue));
    }
  }
