...
export function getSpawnData(config: SpawnConfigurationState, useLocationHash = false): {
  spawnPosition: Vect3;
  ...
  spawnPosition.set(
    randomWithVariance(
      ...
      config.spawnPositionVariance.x,
    ),
    randomWithVariance(
      config.spawnPosition.y,
      config.spawnPositionVariance.y,
    ),
    randomWithVariance(
      config.spawnPosition!.z,
      config.spawnPositionVariance.z,
    ),
  );
  const spawnRotation = new EulXYZ(
    0,
    -config.spawnYRotation! * (Math.PI / 180),
    0,
  );
