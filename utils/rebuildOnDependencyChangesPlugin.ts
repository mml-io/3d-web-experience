...
function findPackageJsonPath(startPath: string): string | null {
  ...
  if (currentPath.includes('node_modules')) {
    ...
    const afterNodeModules = currentPath.substring(nodeModulesIndex + 'node_modules'.length + 1);
    ...
    if (pathParts[0].startsWith('@')) {
      ...
      packagePath = resolve(currentPath.substring(0, nodeModulesIndex), 'node_modules', pathParts[0], pathParts[1]);
    } else {
      // Regular package
      packagePath = resolve(currentPath.substring(0, nodeModulesIndex), 'node_modules', pathParts[0]);
    }
    ...
    if (packageJsonPath) {
      ...
      for (const dep of dependencies) {
        try {
          ...
          if (resolvedPath.includes("/") && !resolvedPath.includes("node_modules/.bin")) {
...
export const rebuildOnDependencyChangesPlugin = (options: RebuildOnDependencyChangesPluginOptions = {}) => {
  ...
  const { watchTransitiveDependencies = !!options.startCommand, maxDepth = 5 } = options;
