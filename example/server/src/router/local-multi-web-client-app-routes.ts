...
const webClientBuildDir = path.join(dirname, "../../local-multi-web-client/build/");
export function addLocalMultiWebAppRoutes(app: enableWs.Application) {
  // Serve frontend statically in production
  const demoIndexContent = fs.readFileSync(path.join(webClientBuildDir, "index.html"), "utf8");
  app.get("/local-multi-web-client/", (req, res) => {
  ...
  app.use("/local-multi-web-client/", express.static(webClientBuildDir));
  ...
  if (process.env.NODE_ENV !== "production") {
    ...
    app.ws("/local-multi-web-client-build", (ws: WebSocket) => {
      const headers = {
